...
tribble(~Ref, ~Gender,  ~CSC, ~RCI, ~refAlpha, ~refSD,
        ...
        "Evans et al., 2002 (UK); CORE-OM (Functioning)", "M", 1.29, 0.84, NA, NA,
        "Evans et al., 2002 (UK); CORE-OM (Risk)",        "M", 0.43, 0.95, NA, NA,
        "Evans et al., 2002 (UK); CORE-OM (Non-risk)",    "M", 1.36, 0.55, NA, NA,
        "Evans et al., 2002 (UK); CORE-OM (all items)",   "F", 1.29, 0.51, NA, NA,
        "Evans et al., 2002 (UK); CORE-OM (WB)",          "F", 1.77, 1.33, NA, NA,
        "Evans et al., 2002 (UK); CORE-OM (Problems)",    "F", 1.62, 0.85, NA, NA,
        "Evans et al., 2002 (UK); CORE-OM (Functioning)", "F", 1.30, 0.84, NA, NA,
        "Evans et al., 2002 (UK); CORE-OM (Risk)",        "F", 0.30, 0.95, NA, NA,
        ...
        "Palmieri et al., 2009; CORE-OM (Functioning)",   "M", 1.29, 0.89, NA, NA,
        "Palmieri et al., 2009; CORE-OM (Risk)",          "M", 0.25, 0.83, NA, NA,
        "Palmieri et al., 2009; CORE-OM (Non-risk)",      "M", 1.27, 0.60, NA, NA,
        "Palmieri et al., 2009; CORE-OM (all items)",     "F", 1.20, 0.52, NA, NA,
        "Palmieri et al., 2009; CORE-OM (WB)",            "F", 1.84, 1.45, NA, NA,
...
telemetry <- Telemetry$new(app_name = "CORE-OM_2_scores",
...
ui <- fluidPage(
  ...
  h1(HTML("App to use dataset of two adult measure CORE scores each from same person")),
  ...
  mainPanel(
    tabsetPanel(type = "tabs",
                ...
                tabPanel("Upload",
                         ...
                         p("This app takes CORE-OM (or adult short form) data in rows each with one or two scores.  These will typically be one row per ",
                         ...
                         tags$ul(
                           tags$li(a("Libre/OpenOffice format (.ods)",
                                     href="https://www.coresystemtrust.org.uk/wp-content/uploads/2025/06/CORE_wide2.ods")),
                           tags$li(a("Excel format (.xlsx)",
                                     href="https://www.coresystemtrust.org.uk/wp-content/uploads/2025/06/CORE_wide2.xlsx"),
                                   " or "),
                           tags$li(a("CSV format (.csv)",
                                     href="https://www.coresystemtrust.org.uk/wp-content/uploads/2025/06/CORE_wide2.csv")),
                           tags$li(a("R data format (.Rda)",
                                     href="https://www.coresystemtrust.org.uk/wp-content/uploads/2025/06/CORE_wide2.rda")),
                         ),
                ...
                tabPanel("Data checking", 
                         ...
                         tags$ul(
                           tags$li("Check measure names and scoring"),
                ...
                tabPanel("Data", 
                         ...
                         tags$ul(tags$li("At the right of  the table the coding gives you the CSC categories of the CORE scores (if there were any.)"),
                                 tags$li("score1toCSC is the difference between the first score and the CSC for that age and gender."),
                                 tags$li("score2toCSC is the same for the second score.  These will get used in the scaled Jacobson plot later."),
                ...
                tabPanel("Scores",
                         ...
                         p("This is the todo list for this tab as I see it at this point"),
                         tags$ul(
                           tags$li("Need to add handling of different scores"),
                ...
                tabPanel("Change (a)",
                         ...
                         p("A cat's cradle plot ignores any respondent with only one usable score and shows ",
                           ...
                           "giving you the scores, the respondent ID, therapist ID and the category ",
                           "of the CSC change."),
                         h2("Todo list"),
                         p("This is the todo list for this tab as I see it at this point"),
                         tags$ul(
                ...
                tabPanel("Jacobson plot",
                         ...
                         p("Points lying on the leading diagonal mark clients ",
                           "whose last score is exactly the same as their first score.  Points below the leading diagonal have lower ",
                           "last scores than their first scores, points above the leading diagonal, ",
                           tags$i("vice versa", .noWS = "after"),
                           "."),
                         p("As the 'tramlines' either side of the leading diagonal always mark the RCI for that person's age and gender.  ",
                           ...
                           "whose scores improved more than the RCI and those above the upper tramline deteriorated more than the RCI."),
                         ...
                         p("As in previous plots you can identify the points by hovering over them ",
                           "when you will see a 'tooltip'",
                           "giving you the person's gender, age, first and last scores (not rescaled), the respondent ID and therapist ID."),
                         h2("Todo list"),
                         p("This is the todo list for this tab as I see it at this point"),
                         tags$ul(
                           tags$li("Need to add handling of different scores"),
                ...
                tabPanel("Explanation of the app",
                         value = 13,
                         h2("Explanation"),
                         p("This app was developed from the YP-CORE_2_scores app converting it for the adult CORE measures."),
                         p("As well as automating lookup of RCSC referential values and allowing the user to download their data with the ",
                         ...
                         p("This is the todo list for this tab as I see it at this point"),
                         tags$ul(
                           tags$li("Need to add remaining referential values and check them all."),
                ...
                tabPanel("Background", 
                         value = 14,
                         p("App created 13.vii.25 by Chris Evans working from the YP-CORE_2_scores app.",
                           a("PSYCTC.org",href="https://www.psyctc.org/psyctc/about-me/")),
                         p("Last updated 13.vii.25: started conversion from YP-CORE to generic adult CORE: tricky!!"),
                         p("Licenced under a ",
...
server <- function(input, output, session) {
  ...
  checkScore <- function(score, Scoring) {
    if(score < 0) {
      return(FALSE)
    }
  ...
  fullData <- reactive({
    ...
    dataInput %>%
      ...
      ### errors in scores
      mutate(meanTxt1 = mean1, # get original character inputs
             meanTxt2 = mean2,
             clinTxt1 = clin1,
             clinTxt2 = clin2,
             ### convert to numeric
             ...
             clin1 = as.numeric(clin1),
             clin2 = as.numeric(clin2),
             ### convert out of range values to missing
             mean1 = if_else(mean1 > 4 | mean1 < 0,
                               NA_real_,
                               mean1),
             mean2 = if_else(mean2 > 4 | mean2 < 0,
                               NA_real_,
                               mean2),
             clin1 = if_else(clin1 > 40 | clin1 < 0,
                               NA_real_,
                               clin1),
             clin2 = if_else(clin1 > 40 | clin2 < 0,
                               NA_real_,
                               clin2)) %>%
      ### now create the score error flags
      mutate(errScore1 = if_else(meanTxt1 != "" & is.na(mean1),
                                 1,
                                 0),
             errMesg = if_else(meanTxt1 != "" & errScore1 == 1,
                               "Impossible entry for mean1",
                               ""),
             errScore1 = if_else(clinTxt1 != "" & is.na(clin1),
                                 1,
                                 errScore1),
             errMesg = if_else(clinTxt1 != "" & errScore1 == 1,
                               "Impossible entry for clin1",
                               errMesg),
             errScore2 = if_else(meanTxt2 != "" & is.na(mean2),
                                 1,
                                 0),
             errMesg = if_else(meanTxt2 != "" & errScore2 == 1,
                               "Impossible entry for mean2",
                               errMesg),
             errScore2 = if_else(clinTxt2 != "" & is.na(clin2),
                                 1,
                                 errScore2),
             errMesg = if_else(clinTxt2 != "" & errScore2 == 1,
                               "Impossible entry for clin2",
                               errMesg),
             ### and now both scores entered but not compatible
             errScore1 = if_else((!is.na(mean1) & !is.na(clin1)) &
                                   abs(10 * mean1 - clin1) > .00001,
                                 1,
                                 errScore1),
             errScore2 = if_else((!is.na(mean2) & !is.na(clin2)) &
                                   abs(10 * mean2 - clin2) > .00001,
                                 1,
                                 errScore2),
             errMesg = if_else((!is.na(mean2) & !is.na(clin2)) &
                                 abs(10 * mean2 - clin2) > .00001,
                               "Mean and clin scores given for second score but incompatible values",
                               errMesg)) -> dataInput 
    dataInput %>%
      ### get the corresponding scorings where we can
      mutate(mean1 = if_else(is.na(mean1) & !is.na(clin1),
                               clin1 / 10,
                               mean1),
             mean2 = if_else(is.na(mean2) & !is.na(clin2),
                               clin2 / 10,
                               mean2),
             clin1 = if_else(is.na(clin1) & !is.na(mean1),
                               mean1 * 10,
                               clin1),
             clin2 = if_else(is.na(clin2) & !is.na(mean2),
                               mean2 * 10,
                               clin2)) %>%
    ...
    ### create variables score1 and score2 using the scoring the user requested
    if (input$Scoring == "Item mean (range 0-4)") {
      dataInput %>%
        mutate(score1 = mean1,
               score2 = mean2) -> dataInput
    } else {
      dataInput %>%
        mutate(score1 = clin1,
               score2 = clin2) -> dataInput
    }
    ...
    dataInput %>%
      mutate(missRespID = as.numeric(is.na(RespondentID)),
             ...
             missscore1 = as.numeric(is.na(score1)),
             missscore2 = as.numeric(is.na(score2)),
    ...
    dataInput %>%
      ...
      mutate(Change = score2 - score1,
             ...
             score1toCSC = score1 - CSC,
             score2toCSC = score2 - CSC,
             scoreScaled1toCSC = score1toCSC / RCI,
             scoreScaled2toCSC = score2toCSC / RCI,
             CSCcat1 = if_else(mean1 > CSC,
                               "High",
                               "Low"),
             CSCcat2 = if_else(mean2 > CSC,
                               "High",
                               "Low"),
             CSCchange = case_when(
  ...
  longDat <- reactive({
    ...
    fullData() %>%
      pivot_longer(cols = starts_with("score"), names_to = "WhichScore", values_to = "Score") 
  })
  ...
  displayData1 <- reactive({
    ...
    fullData() %>%
      ...
      mutate(score1 = round(score1, input$dp),
             score2 = round(score2, input$dp),
             Change = round(Change, input$dp),
             score1toCSC = round(score1toCSC, input$dp),
             score2toCSC = round(score2toCSC, input$dp),
             scoreScaled1toCSC = round(scoreScaled1toCSC, input$dp),
             scoreScaled2toCSC = round(scoreScaled2toCSC, input$dp),)
  })
  ...
  tibSummaryStatsWide <- reactive({
    ...
    fullData() %>%
      summarise(totalRecords = n(),
                ...
                ### scores
                nScore1valid = getNOK(score1),
                nScore2valid = getNOK(score2),
                nScorebothValid = sum(!is.na(score1) & !is.na(score2)),
                minScore1 = min(score1, na.rm = TRUE),
                meanScore1 = mean(score1, na.rm = TRUE),
                medianScore1 = median(score1, na.rm = TRUE),
                maxScore1 = max(score1, na.rm = TRUE),
                sdScore1 = sd(score1, na.rm = TRUE),
                minScore2 = min(score2, na.rm = TRUE),
                meanScore2 = mean(score2, na.rm = TRUE),
                medianScore2 = median(score2, na.rm = TRUE),
                maxScore2 = max(score2, na.rm = TRUE),
                sdScore2 = sd(score2, na.rm = TRUE),
  ...
  errorDataTab <- reactive({
    fullData() %>%
      filter(nErrVals > 0) %>%
      # select(RespondentID : clin2, starts_with("nSessions"), starts_with("err"), nErrVals)
      select(RespondentID, TherapistID, mean1, mean2, clin1, clin2, Start_date, End_date,
             starts_with("err"))
  })
  ...
  missingDataTab <- reactive({
    fullData() %>%
      filter(nMissVals > 0) %>%
      select(RespondentID : clin2, starts_with("nSessions"), nMissVals)
  })
  ...
  output$downloadClinician1CSV <- downloadHandler(
    filename = "CORE_clinician_data.csv",
    contentType = "text/csv",
    content = function(file) {
  ...
  output$downloadClinician1XLSX <- downloadHandler(
    filename = "CORE_clinician_data.xlsx",
    contentType = "text/csv",
    content = function(file) {
  ...
  output$downloadClinician1ODS <- downloadHandler(
    filename = "CORE_clinician_data.ods",
    contentType = "text/csv",
    content = function(file) {
  ...
  catsCradle1 <- reactive({
    ...
    longDat() %>%
      ...
      mutate(WhichScore = str_remove(WhichScore, "score"),
  ...
  catsCradle2 <- reactive({
    ...
    ggplot(data = tmpTib,
           aes(x = Start_date, y = score1, group = RespondentID, colour = TherapistID,
               label1 = RespondentID,
               label2 = Gender,
               label3 = score1,
               label4 = score2, 
               label5 = CSCchange)) +
      geom_point(shape = 22) +
      geom_point(aes(x = End_date, y = score2),
                 shape = 23) +
      geom_segment(aes(x = Start_date, xend = End_date,
                       y = score1, yend = score2)) +
      xlab("Date") +
      scale_y_continuous("CORE score",
                         breaks = seq(0,
                                      yLims[2],
  ...
  summaryScoresTxt1  <- reactive({
    ...
    str_c("Across your ",
          ...
          tibSummaryStatsWide()$nScore1valid,
          " usable initial scores, ",
          tibSummaryStatsWide()$nScore2valid,
          " usable t2 scores and ",
          tibSummaryStatsWide()$nScorebothValid,
  ...
  scoreStatsTable1 <- reactive({
    fullData() %>%
      select(score1, score2, Change) %>%
      pivot_longer(cols = everything(),
                   names_to = "Score") %>%
      mutate(Score = case_when(
        Score == "score1" ~ "Occasion 1",
        Score == "score2" ~ "Occasion 2",
        Score == "Change" ~ "Change"),
        Score = ordered(Score,
                        levels = c("Occasion 1",
                                   "Occasion 2",
  ...
  histScores1 <- reactive({
    req(input$file1)
    fullData() %>%
      summarise(mean = mean(score1, na.rm = TRUE),
                median = median(score1, na.rm = TRUE)) -> tmpTibStats
    ...
    ggplot(data = fullData(),
      ...
      geom_vline(xintercept = tmpTibStats$median,
                 colour = "green") +
      scale_x_continuous("First CORE scores",
                         breaks = seq(0, 4, .2),
                         limits = c(0, 4)) 
  })
  ...
  histScores2 <- reactive({
    req(input$file1)
    fullData() %>%
      summarise(mean = mean(score2, na.rm = TRUE),
                median = median(score2, na.rm = TRUE)) -> tmpTibStats
    ...
    ggplot(data = fullData(),
      ...
      geom_vline(xintercept = tmpTibStats$median,
                 colour = "green") +
      scale_x_continuous("Second CORE scores",
                         breaks = seq(0, 4, .2),
                         limits = c(0, 4))
  })
    ...
    fullData() %>%
      mutate(Gender = str_trim(Gender),
             Gender = if_else(is.na(Gender) | Gender == "",
                              "Not recorded",
                              Gender)) %>%
      select(RespondentID, TherapistID, Gender, Age, score1, score2, scoreScaled1toCSC, scoreScaled2toCSC) -> tmpTib
    
    tmpTib %>%
      summarise(min = min(c(min(scoreScaled1toCSC),
                            min(scoreScaled2toCSC))),
                max = max(c(max(scoreScaled1toCSC),
                            max(scoreScaled2toCSC)))) -> tmpTibLimits
    
    ggplot(tmpTib,
           aes(x = scoreScaled1toCSC,
               y = scoreScaled2toCSC,
               colour = Gender,
               label1 = Age,
               label2 = RespondentID,
      ...
      geom_abline(slope = 1, intercept = -1,
                  linetype = 3) +
      scale_x_continuous("First CORE score, rescaled",
                         limits = c(tmpTibLimits$min, 
                                    tmpTibLimits$max)) +
      scale_y_continuous("Second CORE score, rescaled",
                         limits = c(tmpTibLimits$min, 
                                    tmpTibLimits$max)) 
