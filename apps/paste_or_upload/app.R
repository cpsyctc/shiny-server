...
server <- function(input, output, session) {
  ...
  output$pasting <- reactive({
    inputType() == "paste"
  })
  
  outputOptions(output, "pasting", suspendWhenHidden = FALSE)  
  ...
  dataSummary <- reactive({
    ...
    if (input$inputType == "paste") {
      if (getNOK(pastedDataNum()) > 0) {
        tibble(x = pastedDataNum()) %>%
          summarise(n = n(),
                    nNA = getNNA(x),
                    nOK = n - nNA,
      ...
      ### uploaded data
      if (getNOK(selectedData()) > 0) {
        selectedData() %>%
          summarise(n = n(),
                    nNA = getNNA(!!input$var),
                    nOK = n - nNA,
