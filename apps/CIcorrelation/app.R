...
#   titlePanel("Parametric confidence interval for Cronbach alpha"),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      p("This uses parametric assumptions and the assumption and that it is a fair indication of the CI for Spearman's rho\n
        as well as for the Pearson R because, if there were no ties and underlying Gaussian distributions, then rho is the\n
...
server <- function(input, output) {
  ...
  output$res <- renderText({
    validate(
      ...
      need(checkNumRange(input$ci, minx = -1, maxx = 1, incEq = TRUE),
           "ci must be a value >= -1.0 and <= 1.0"),
